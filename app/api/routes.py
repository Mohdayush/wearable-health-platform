...
def register_device(payload: DeviceCreate, db: Session = Depends(get_db), user: User = Depends(current_user)):
    device = Device(user_id=user.id, name=payload.name.strip(), model=payload.model.strip(), device_token=secrets.token_urlsafe(32))
...
def set_device_status(device_id: int, active: bool, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    device.active = active; db.commit()
...
def ingest_measurement(device_id: int, payload: MeasurementCreate, x_device_token: str = Header(), db: Session = Depends(get_db)):
    ...
    now = datetime.now(timezone.utc).replace(tzinfo=None); observed_at = payload.observed_at.replace(tzinfo=None)
    if observed_at > now + timedelta(minutes=5): raise HTTPException(status_code=422, detail="Future timestamps are not accepted")
    ...
    try:
        db.add(point); db.flush(); db.commit()
    except IntegrityError:
        db.rollback(); return {"status": "duplicate_ignored"}
    ...
    if alert:
        ...
        db.add(Alert(user_id=device.user_id, device_id=device.id, metric=point.metric, value=point.value, severity=severity, message=message)); db.commit()
...
def dashboard(db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    return {"user": UserResponse.model_validate(user), "metrics": dashboard_snapshot(db, user.id), "devices": [DeviceResponse.model_validate(d) for d in devices], "alerts": [AlertResponse.model_validate(a) for a in alerts]}
...
def health_summary(metric: str = Query(...), hours: int = 24, db: Session = Depends(get_db), user: User = Depends(current_user)):
    if not 1 <= hours <= 720: raise HTTPException(status_code=422, detail="hours must be between 1 and 720")
    end = datetime.utcnow(); return user_metric_summary(db, user.id, metric, end - timedelta(hours=hours), end)
...
def acknowledge_alert(alert_id: int, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    alert.acknowledged = True; db.commit(); return {"status": "acknowledged"}
...
def set_alert_preference(payload: AlertPreferenceRequest, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    if pref: pref.lower_threshold, pref.upper_threshold = payload.lower_threshold, payload.upper_threshold
    else: db.add(AlertPreference(user_id=user.id, **payload.model_dump()))
    db.commit(); return {"status": "saved", "metric": payload.metric}
...
def assistant_chat(payload: AssistantRequest, db: Session = Depends(get_db), user: User = Depends(current_user)):
    text = payload.message.lower(); metrics = dashboard_snapshot(db, user.id)
    if "heart" in text or "pulse" in text:
        item=metrics["heart_rate"]; answer=f"Your last 24h heart-rate data contains {item['count']} readings. The latest value is {item['latest_value']} {item['unit']} and the average is {item['average']} {item['unit']}."
    elif "oxygen" in text or "spo2" in text:
        item=metrics["spo2"]; answer=f"Your last 24h SpO₂ data contains {item['count']} readings. The latest value is {item['latest_value']}% and the average is {item['average']}%."
    elif "temperature" in text:
        item=metrics["temperature"]; answer=f"Your last 24h temperature data contains {item['count']} readings. The latest value is {item['latest_value']}°C and the average is {item['average']}°C."
    elif "step" in text or "activity" in text:
        item=metrics["steps"]; answer=f"Your last 24h activity data contains {item['count']} readings. The latest step count reading is {item['latest_value']}."
    else: answer="I can summarize your recent heart rate, SpO₂, temperature, and activity data. Ask me about one of those metrics."
