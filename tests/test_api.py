...
def test_auth_device_and_dashboard_flow():
    ...
    measurement = client.post(f"/api/v1/devices/{device_data['id']}/measurements", headers={"X-Device-Token": device_data["device_token"]}, json={"metric":"heart_rate","value":72,"unit":"bpm","observed_at":datetime.now(timezone.utc).isoformat(),"idempotency_key":uuid4().hex})
    ...
    assert "heart" in assistant.json()["answer"].lower()
